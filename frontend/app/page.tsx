    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
            ...
            {isSignedIn ? (
              <Link href="/chat">
                <Button size="sm" className="text-[13px] bg-white text-neutral-900 hover:bg-neutral-200">
                  Open Townhall <ArrowRight className="ml-1 h-3.5 w-3.5" />
                </Button>
              </Link>
            ) : (
              <>
                <Link href="/sign-in">
                  <Button variant="ghost" size="sm" className="text-[13px] text-neutral-400 hover:text-white">Sign in</Button>
                </Link>
                <Link href="/sign-up">
                  <Button size="sm" className="text-[13px] bg-white text-neutral-900 hover:bg-neutral-200">Get started</Button>
                </Link>
              </>
            )}
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr,1.1fr] gap-16 items-start">
          <div>
              ...
              {isSignedIn ? (
                <Link href="/chat">
                  <Button size="lg" className="h-11 px-6 text-[14px] bg-white text-neutral-900 hover:bg-neutral-200">
                    Open Townhall <ArrowRight className="ml-1.5 h-4 w-4" />
                  </Button>
                </Link>
              ) : (
                <>
                  <Link href="/sign-up">
                    <Button size="lg" className="h-11 px-6 text-[14px] bg-white text-neutral-900 hover:bg-neutral-200">
                      Try it now <ArrowRight className="ml-1.5 h-4 w-4" />
                    </Button>
                  </Link>
                  <Link href="/sign-in">
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {["Next.js 16", "FastAPI", "DynamoDB", "SSE", "Clerk Auth", "S3", "Tailwind"].map((t) => (
            ...
            <div className="flex h-[400px]">
              <div className="w-[180px] bg-neutral-900 border-r border-neutral-800 p-3">
                ...
                <div className="space-y-0.5">
                  {[{ ch: "general", active: true }, { ch: "engineering", active: false, unread: 3 }, { ch: "design", active: false }, { ch: "shipped", active: false }].map((c) => (
                    <div key={c.ch} className={`flex items-center gap-1.5 px-2 py-1 rounded text-[11px] ${c.active ? "bg-neutral-700 text-white" : "text-neutral-500"}`}>
                      <Hash className="h-2.5 w-2.5 shrink-0 opacity-50" />
                      <span className="flex-1">{c.ch}</span>
                      {c.unread && <span className="flex h-[14px] min-w-[14px] items-center justify-center rounded-full bg-red-500 text-white text-[8px] font-bold px-0.5">{c.unread}</span>}
                    </div>
                  ))}
                </div>
              </div>
              <div className="flex-1 flex flex-col bg-neutral-950">
                ...
                <div className="flex-1 p-3 space-y-3 overflow-hidden">
                  {[
                    { name: "Sarah", msg: "Just shipped the new onboarding flow!", time: "10:32", color: "bg-blue-500/20 text-blue-400" },
                    { name: "Alex", msg: "The animations are smooth. Nice work.", time: "10:34", color: "bg-amber-500/20 text-amber-400" },
                    { name: "Jordan", msg: "Can we add that pattern to the design system?", time: "10:35", color: "bg-emerald-500/20 text-emerald-400" },
                  ].map((m, i) => (
                ...
                <div className="p-2 border-t border-neutral-800">
                  <div className="rounded-lg border border-neutral-800 bg-neutral-900 px-2.5 py-1.5 text-[11px] text-neutral-600 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 text-[9px] text-neutral-700"><Command className="h-2.5 w-2.5" /><span>K to search</span></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="border-t border-neutral-800/50">
        <div className="mx-auto max-w-5xl px-6 py-24">
          ...
          <p className="text-[15px] text-neutral-500 mb-12 max-w-lg">Every feature you&apos;d expect from a production chat app.</p>
          <div className="grid sm:grid-cols-2 gap-x-16 gap-y-10">
            {[
              { label: "Real-time SSE", detail: "Messages, typing indicators, reactions, and presence updates stream over Server-Sent Events." },
              { label: "Emoji reactions", detail: "React to any message with quick emoji. Toggle on/off, see who reacted, counts update live." },
              { label: "File uploads", detail: "Drag and drop or click to attach. Text files preview inline, binary files upload to S3." },
              { label: "Message editing", detail: "Edit your own messages inline. Changes broadcast to all clients instantly." },
              { label: "Full-text search", detail: "Cmd+K opens search. Search across all messages, jump to any channel." },
              { label: "Typing indicators", detail: "See who's composing. Debounced, animated dots, auto-expire." },
              { label: "Unread badges", detail: "Red count badges on channels with new messages. Auto-clear on room enter." },
              { label: "Presence tracking", detail: "Online/offline status per user. Heartbeat + 60s staleness detection." },
            ].map((f) => (
              <div key={f.label}>
        ...
        <div className="mx-auto max-w-5xl px-6 py-16">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              ...
              { layer: "Backend", stack: "FastAPI + Python 3.13", note: "REST + SSE endpoints, JWT auth, S3 uploads" },
      ...
      <section className="border-t border-neutral-800/50">
        <div className="mx-auto max-w-5xl px-6 py-20">
          ...
          <p className="mt-2 text-[15px] text-neutral-500 max-w-md">Sign up, create a channel, and start chatting.</p>
          <div className="mt-6">
            <Link href={isSignedIn ? "/chat" : "/sign-up"}>
              <Button size="lg" className="h-11 px-8 text-[14px] bg-white text-neutral-900 hover:bg-neutral-200">
                {isSignedIn ? "Go to Townhall" : "Get started free"} <ArrowRight className="ml-1.5 h-4 w-4" />
              </Button>
            </Link>
          </div>
        </div>
